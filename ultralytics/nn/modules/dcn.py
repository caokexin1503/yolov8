...
class DCN(nn.Module):
    """Pure PyTorch Deformable Convolution No mmcv No torchvision deform_conv2d.
    """
    ...
    def forward(self, x):
        ...
        # output coordinate
        yy, xx = torch.meshgrid(torch.arange(Hout, device=device), torch.arange(Wout, device=device), indexing="ij")
        ...
        sampled_feat = torch.stack(output, dim=2)

        sampled_feat = sampled_feat.view(B, C * self.k * self.k, Hout, Wout)

        weight = self.weight.view(self.c2, -1, 1, 1)

        out = F.conv2d(sampled_feat, weight, bias=self.bias)
...
class DCNConv(nn.Module):
    def __init__(self, c1, c2, k=3, s=1, p=None):
        ...
        self.conv = DCN(c1, c2, k=k, s=s, p=p)
