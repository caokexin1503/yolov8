...
class BboxLoss(nn.Module):
    ...
    def forward(
        ...
        # 测试是否使用了SIoU
        # print(f"[DEBUG] iou_type = {self.iou_type}")
        iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, **flags)
...
class v8DetectionLoss:
    ...
    def __init__(
        ...
        self.device = device
        self.qfl_beta = 2.0  # 新增！用于quality focal loss
    ...
    def get_assigned_targets_and_loss(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> tuple:
        ...
        if fg_mask.sum() > 0:
            ...
            stats = {
                ...
                "num_anchors": fg_mask.numel(),
                "fg_ratio": (fg_mask.sum() / fg_mask.numel()).item(),
                "avg_target_score": pos_scores.mean().item(),
                "pos_per_gt_mean": pos_per_gt.mean().item(),
                "pos_per_gt_min": pos_per_gt.min().item(),
                "pos_per_gt_max": pos_per_gt.max().item(),
            }
        else:
            stats = {
                "num_pos": 0,
        ...
        # Cls loss with optional class weighting (now using Focal Loss)
        # focal_loss = self.focal_loss(pred_scores, target_scores.to(dtype))  # (bs, num_anchors, nc)
        # if self.class_weights is not None:
        #     focal_loss *= self.class_weights
        # loss[1] = focal_loss.sum() / target_scores_sum  # Focal Loss

        # Cls loss: Quality Focal Loss
        # qfl_loss = quality_focal_loss(pred_scores, target_scores.to(dtype), beta=self.qfl_beta)
        # if self.class_weights is not None:
